        ...
        async with await open_transport(self.transport) as hci_transport:
            ...
            def on_pdu(pdu: HCI_IsoDataPacket, ase: bap.AseStateMachine):
                ...
                pcm = decode(
                    ...
                    codec_config.audio_channel_allocation.num_channels,
                    pdu.iso_sdu_fragment,
                )
                self.device.abort_on('disconnection', self.ui_server.send_audio(pcm))

            def on_ase_state_change(
                state: bap.AseStateMachine.State,
                ase: bap.AseStateMachine,
            ) -> None:
                if state == bap.AseStateMachine.State.STREAMING:
                    codec_config = ase.codec_specific_configuration
                    assert isinstance(codec_config, bap.CodecSpecificConfiguration)
                    assert ase.cis_link
                ...
                elif state == bap.AseStateMachine.State.CODEC_CONFIGURED:
                    codec_config = ase.codec_specific_configuration
                    assert isinstance(codec_config, bap.CodecSpecificConfiguration)
                    if ase.role == bap.AudioRole.SOURCE:
                        setup_encoders(
