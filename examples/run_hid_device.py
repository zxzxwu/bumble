...
class HidDeviceDelegate(hid.Device.Delegate):
    ...
    def get_report(self, report_type: hid.ReportType, report_id: int | None) -> bytes:
        ...
        if report_type == hid.ReportType.OUTPUT_REPORT:
            # Return single byte sample data for testing
            return bytes([0x11])
        if report_type == hid.ReportType.FEATURE_REPORT:
            raise hid.HidProtocolError(
                hid.HandshakeMessage.ResultCode.ERR_INVALID_PARAMETER
            )
        if report_type == hid.ReportType.OTHER_REPORT and report_id == 3:
            raise hid.HidProtocolError(
                hid.HandshakeMessage.ResultCode.ERR_INVALID_REPORT_ID
            )
        raise hid.HidProtocolError(
    ...
    def set_report(self, report_type: hid.ReportType, data: bytes) -> None:
        ...
        if report_type == hid.ReportType.INPUT_REPORT:
            if report_id == 1 and len(data) != len(device_data.keyboard_data):
                raise hid.HidProtocolError(
                    hid.HandshakeMessage.ResultCode.ERR_INVALID_PARAMETER
                )
            if report_id == 2 and len(data) != len(device_data.mouse_data):
                raise hid.HidProtocolError(
                    hid.HandshakeMessage.ResultCode.ERR_INVALID_PARAMETER
                )
            if report_id == 3:
            ...
            while True:
                ...
                if choice == "1":
                    ...
                    if choice1 == "1":
                        ...
                        hid_device.send_interrupt_data(bytes([0x02, 0x00, 0x00, 0xF6]))
                        hid_device.send_interrupt_data(bytes([0x02, 0x00, 0x00, 0x00]))

                    elif choice1 == "3":
                        hid_device.send_interrupt_data(bytes([0x00, 0x00, 0x00, 0x00]))
                        hid_device.send_interrupt_data(bytes([0x00, 0x00, 0x00, 0x00]))
