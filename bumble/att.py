...
from typing import Dict, Type, List, Protocol, TYPE_CHECKING
...
class Attribute(EventEmitter):
    class Permission(enum.IntFlag):
        ...
        @classmethod
        def from_string(cls, permissions_str: str) -> Attribute.Permission:
            try:
                return functools.reduce(
                    lambda x, y: x | Attribute.Permission[y],
                    permissions_str.split(","),
                    Attribute.Permission(0),
                )
    ...
    READ_REQUIRES_ENCRYPTION = Permission.READ_REQUIRES_ENCRYPTION
    WRITE_REQUIRES_ENCRYPTION = Permission.WRITE_REQUIRES_ENCRYPTION
    READ_REQUIRES_AUTHENTICATION = Permission.READ_REQUIRES_AUTHENTICATION
    WRITE_REQUIRES_AUTHENTICATION = Permission.WRITE_REQUIRES_AUTHENTICATION
    READ_REQUIRES_AUTHORIZATION = Permission.READ_REQUIRES_AUTHORIZATION
    WRITE_REQUIRES_AUTHORIZATION = Permission.WRITE_REQUIRES_AUTHORIZATION

    value: int | bytes | ConnectionValue

    def __init__(
        self,
        attribute_type: str | bytes | UUID,
        permissions: str | Permission,
        value: int | str | bytes | ConnectionValue = b'',
    ) -> None:
        EventEmitter.__init__(self)
        self.handle = 0
        self.end_group_handle = 0
        if isinstance(permissions, str):
            self.permissions = Attribute.Permission.from_string(permissions)
        else:
